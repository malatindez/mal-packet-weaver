...
namespace mal_packet_weaver::crypto::ECDSA
{
     ...
     */
    [[nodiscard]] inline int GetCurveByName(const std::string_view curve)
    {
        if (curve == "secp256k1")
        {
        ...
        AlwaysAssert(false, "Unknown curve type");
     ...
     */
    class KeyPairGenerator : non_copyable_non_movable
    {
    public:
        /**
        ...
        explicit KeyPairGenerator(const int curve_id)
        {
            ...
            AlwaysAssert(ctx_ != nullptr, "EVP_PKEY_CTX_new_id() failed");
            AlwaysAssert(EVP_PKEY_keygen_init(ctx_.get()) > 0, "EVP_PKEY_keygen_init() failed");
            AlwaysAssert(EVP_PKEY_CTX_set_ec_paramgen_curve_nid(ctx_.get(), curve_id) > 0,
        ...
        [[nodiscard]] KeyPair generate() const
        {
            EVP_PKEY_WRAPPER pkey;
            {
                ...
                AlwaysAssert(EVP_PKEY_keygen(ctx_.get(), &pkey_) > 0, "EVP_PKEY_keygen() failed");
                pkey.reset(pkey_);
            }
            ...
            AlwaysAssert(PEM_write_bio_PrivateKey(bio.get(), pkey.get(), nullptr, nullptr, 0, nullptr, nullptr) > 0,
            ...
            AlwaysAssert(bio != nullptr, "BIO_new_file() failed");
            AlwaysAssert(PEM_write_bio_PUBKEY(bio.get(), pkey.get()) > 0, "PEM_write_bio_PUBKEY() failed");
            key_size = BIO_get_mem_data(bio.get(), &key_data);
            Key public_key;
     ...
     */
    class Signer : non_copyable_non_movable
    {
    public:
        /**
        ...
        Signer(const KeyView private_key, const Hash::HashType hash_type) : hash_type_(hash_type)
        {
            ...
            AlwaysAssert(pkey_ != nullptr, "PEM_read_bio_PrivateKey() failed");

            ctx_.reset(EVP_PKEY_CTX_new(pkey_.get(), nullptr));
            AlwaysAssert(ctx_ != nullptr, "EVP_PKEY_CTX_new() failed");
            AlwaysAssert(EVP_PKEY_sign_init(ctx_.get()) > 0, "EVP_PKEY_sign_init() failed");
            if (hash_type == Hash::HashType::SHA256)
            {
                AlwaysAssert(EVP_PKEY_CTX_set_signature_md(ctx_.get(), EVP_sha256()) > 0,
                             "EVP_PKEY_CTX_set_signature_md() failed");
            }
            else if (hash_type == Hash::HashType::SHA384)
            {
                AlwaysAssert(EVP_PKEY_CTX_set_signature_md(ctx_.get(), EVP_sha384()) > 0,
                             "EVP_PKEY_CTX_set_signature_md() failed");
            }
            else if (hash_type == Hash::HashType::SHA512)
            {
                AlwaysAssert(EVP_PKEY_CTX_set_signature_md(ctx_.get(), EVP_sha512()) > 0,
                             "EVP_PKEY_CTX_set_signature_md() failed");
            }
            else
            {
                AlwaysAssert(false, "Unsupported hash type");
            }
        }
        ...
        {
            Assert(hash.hash_type == hash_type_, "Unsupported hash type");

            size_t signature_size = 0;
            ByteArray signature;

            AlwaysAssert(EVP_PKEY_sign(ctx_.get(), nullptr, &signature_size, hash.as_uint8(), hash.size()) > 0,
                         "EVP_PKEY_sign() failed");
            signature.resize(signature_size);

            AlwaysAssert(EVP_PKEY_sign(ctx_.get(), signature.as<unsigned char>(), &signature_size, hash.as_uint8(),
                                       hash.size()) > 0,
                         "EVP_PKEY_sign() failed");
            signature.resize(signature_size);
            return signature;
     ...
     */
    class Verifier : non_copyable_non_movable
    {
    public:
        /**
        ...
        Verifier(const KeyView public_key, const Hash::HashType hash_type) : hash_type_(hash_type)
        {
            ...
            AlwaysAssert(pkey_ != nullptr, "PEM_read_bio_PUBKEY() failed");

            ctx_.reset(EVP_PKEY_CTX_new(pkey_.get(), nullptr));
            AlwaysAssert(ctx_ != nullptr, "EVP_PKEY_CTX_new() failed");
            AlwaysAssert(EVP_PKEY_verify_init(ctx_.get()) > 0, "EVP_PKEY_verify_init() failed");
            if (hash_type == Hash::HashType::SHA256)
            {
                AlwaysAssert(EVP_PKEY_CTX_set_signature_md(ctx_.get(), EVP_sha256()) > 0,
                             "EVP_PKEY_CTX_set_signature_md() failed");
        ...
        [[nodiscard]] bool verify_hash(const Hash hash, const ByteView signature) const
        {
            ...
            return EVP_PKEY_verify(ctx_.get(), signature.as<unsigned char>(), signature.size(), hash.as_uint8(),
                                   hash.size()) > 0;
        }
     ...
     */
    [[deprecated]] [[nodiscard]] inline KeyPair generate_key_pair(std::string curve_name)
    {
     ...
     * @see Signer
     */
    [[deprecated]] [[nodiscard]] inline ByteArray sign_data(const KeyView private_key, const ByteView data,
     ...
     * @see Signer
     */
    [[deprecated]] [[nodiscard]] inline ByteArray sign_hash(const KeyView private_key, const Hash hash)
    {
     ...
     * @see Verifier
     */
    [[deprecated]] [[nodiscard]] inline bool verify_data(const KeyView public_key, const ByteView data,
                                                         const ByteView signature, const Hash::HashType hash_type)
    {
     ...
     * @see Verifier
     */
    [[deprecated]] [[nodiscard]] inline bool verify_hash(const KeyView public_key, const Hash hash,
